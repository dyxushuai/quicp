...
fn platform_bridge_exposes_bounded_complete_ip_packet_seam() {
    ...
    let mut device = bridge
        ...
        .expect("device");
    let (rx, _tx) = device.receive(Instant::ZERO).expect("ingress packet");
...
#[test]
fn caller_owned_egress_keeps_the_preallocated_pool_live() {
...
fn platform_bridge_serializes_parallel_ingress_calls() {
    ...
    for _ in 0..64 {
        let (rx, _tx) = device.receive(Instant::ZERO).expect("ingress packet");
        rx.consume(|packet| assert_eq!(packet.len(), 64));
    }
