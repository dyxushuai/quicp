//! A deployable Unix SOCKS5 CONNECT proxy carried through `FakeTCP` QUICP flows.
//!
...
#[cfg(unix)]
use quicp::{
    CanonicalHost, CarrierConfig, Client, ClientConfig, CongestionControl, Connection, FlowError,
    FourTuple, Multipath, OpenRequest, OpenStatus, PathCandidate, PendingFlow, Server,
    ServerConfig, SynDataPolicy,
};
#[cfg(unix)]
...
#[cfg(unix)]
const DEFAULT_SOCKS5_LISTEN: SocketAddr = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 1_080);
#[cfg(unix)]
const CONNECT_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(10);
#[cfg(unix)]
const USAGE: &str = "Usage:\n  socks5_tunnel client --local <client-ip:port> --server <server-ip:port> [--socks5 <listen>] --secret <absolute-file>\n  socks5_tunnel server --listen <server-ip:port> --client <client-ip:port> --secret <absolute-file>\n\nBoth processes require CAP_NET_RAW and tuple-scoped TCP RST suppression.\n";

#[cfg(not(unix))]
fn main() {
    eprintln!(
        "socks5_tunnel requires a Unix raw FakeTCP carrier; use examples/host_loopback.rs for a portable carrier demo"
    );
}
...
async fn run_client(args: Args) -> Result<(), Box<dyn Error>> {
    ...
    let path = PathCandidate::new("primary", args.local.ip(), args.peer)?;
...
async fn handle_socks5_client(
    ...
    let mut flow =
        match tokio::time::timeout(CONNECT_TIMEOUT, connection.open_flow(request, true)).await {
            Ok(Ok(flow)) => flow,
            Ok(Err(error)) => {
