...
impl Future for ConnectionDriver {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let mut keep_going = match conn.drive_transmit(cx) {
            ...
            Err(error) => {
                let transport_error = TransportError::new(
                    TransportErrorCode::INTERNAL_ERROR,
                    format!("socket send failed: {error}"),
                );
                conn.inner.abort_io(transport_error);
        ...
        loop {
                ...
                .poll_send(&udp_transmit(&t, &self.send_buffer[..len]), cx)
            {
                ...
                Poll::Ready(Err(error)) if is_path_unavailable(&error) => {
                    let path = FourTuple::new(t.destination, t.src_ip);
                    self.inner
                        .abandon_failed_path(now, path)
                        .map_err(|_| error)?;
                    continue;
                }
