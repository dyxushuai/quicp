...
impl AsyncUdpSocket for SendErrorSocket {
    fn create_sender(&self) -> Pin<Box<dyn UdpSender>> {
        Box::pin(SendErrorSender)
    }
...
#[derive(Debug)]
struct SendErrorSender;

impl UdpSender for SendErrorSender {
    fn poll_send(
        self: Pin<&mut Self>,
...
#[tokio::test]
async fn send_io_error_closes_connection_instead_of_hanging() {
    ...
    let runtime: Arc<dyn crate::Runtime> = Arc::new(TokioRuntime);
    let endpoint = Endpoint::new_with_abstract_socket(
        EndpointConfig::default(),
        None,
        Box::new(SendErrorSocket {
            local: SocketAddr::from((Ipv4Addr::LOCALHOST, 40_000)),
        }),
        runtime,
    )
    .unwrap();
    ...
    let error = tokio::time::timeout(
        ...
        endpoint
            .connect(SocketAddr::from((Ipv4Addr::LOCALHOST, 44_000)), "localhost")
            .unwrap(),
    )
    .await
    .expect("connection future hung after send error")
    .unwrap_err();
    assert!(matches!(error, ConnectionError::TransportError(_)));
}
