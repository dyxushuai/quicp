...
use rand::RngExt;
use tracing::{Level, Span, debug, trace, trace_span};
...
impl<'a, 'b> PacketBuilder<'a, 'b> {
        ...
        'b: 'a,
    {
        ...
        let span = if trace_enabled {
            trace_span!("send", space = ?space_id, pn = packet_number, %path_id).entered()
        } else {
            Span::none().entered()
        };
    ...
    pub(crate) fn simple_data_buf(buf: &'a mut TransmitBuf<'b>) -> Self {
        Self {
            ...
            trace_enabled: tracing::enabled!(Level::TRACE),
        }
    }
