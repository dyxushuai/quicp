...
impl Connection {
    ...
    /// Abort after an unrecoverable local I/O failure.
    pub fn abort_io(&mut self, reason: TransportError) {
        self.kill(ConnectionError::TransportError(reason));
    }

    /// Abandon the path selected for a failed local datagram send.
    pub fn abandon_failed_path(
        &mut self,
        now: Instant,
        network_path: FourTuple,
    ) -> Result<(), ClosePathError> {
        let path_id = self
            ...
            .keys()
            .copied()
            .find(|id| self.network_path(*id) == Ok(network_path))
            .ok_or(ClosePathError::ClosedPath)?;
        let has_usable_alternative = self.paths.iter().any(|(id, path)| {
            *id != path_id
                && self.remote_cids.contains_key(id)
                && !self.abandoned_paths.contains(id)
                && path.data.validated
        });
        if !has_usable_alternative {
            return Err(ClosePathError::LastOpenPath);
        }
    ...
    fn process_payload(
        ...
        let mut ack_eliciting = false;
        let trace_frames = tracing::enabled!(tracing::Level::TRACE);
        ...
        for result in frame::Iter::new(payload)? {
            ...
            let span = if trace_frames {
                trace_span!("frame", ty = %frame_type, path = tracing::field::Empty)
            } else {
                tracing::Span::none()
            };
        ...
        if !scheduling_info.is_abandoned && scheduling_info.may_send_data {
            if space.number_spaces.len() == 1 {
                let ack_path = space
                    .number_spaces
                    .iter()
                    .find_map(|(&path_id, pns)| pns.pending_acks.can_send().then_some(path_id));
                if let Some(path_id) = ack_path {
                    Self::populate_acks(
                        now,
                        self.receiving_ecn,
                        path_id,
